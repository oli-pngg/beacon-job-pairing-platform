...
function CandidateDashboard({ data }: { data: DashboardData }) {
  ...
  return (
    <>
      <div className="page-heading"><div><p className="eyebrow">Your workspace</p><h1>Good to see you, {profileName.split(' ')[0]}.</h1><p>Build momentum one verified skill and one thoughtful application at a time.</p></div><Link className="button" href="/jobs">Explore openings <ArrowUpRight size={16} aria-hidden="true" /></Link></div>
...
function EmployerDashboard({ data }: { data: DashboardData }) {
  ...
  return (
    <>
      ...
      <div className="content-grid">
        <section className="panel" aria-labelledby="openings-heading"><div className="panel-header"><div><h2 id="openings-heading">Your openings</h2><p>Keep requirements specific and accommodation-ready.</p></div><Link className="text-link" href="/employer/jobs">Manage all</Link></div>{jobs.length ? <div className="mini-list">{jobs.slice(0, 5).map((job) => <li key={job.id}><BriefcaseBusiness size={17} aria-hidden="true" /><div><strong>{job.title}</strong><span><StatusPill status={job.status} /> / posted {formatDate(job.created_at)}</span></div></li>)}</div> : <div className="empty-state"><h3>Your first opening starts here.</h3><p>Describe the work, the skills, and the accommodations that make success possible.</p><Link className="button small" href="/employer/jobs/new">Create opening</Link></div>}</section>
