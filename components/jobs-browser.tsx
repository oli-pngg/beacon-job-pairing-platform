...
function JobCard({ job, canApply, onApply }: { job: Job; canApply: boolean; onApply: (jobId: string) => Promise<void> }) {
  ...
  return <article className={`job-card ${job.fit_score && job.fit_score >= 75 ? 'featured' : ''}`}>
    <div className="job-card-header"><div><h2>{job.title}</h2><p className="job-company">{job.organization_name ?? 'Beacon employer'}</p><p>{job.description}</p></div>{typeof job.fit_score === 'number' ? <div className="fit-score large" aria-label={`${job.fit_score} percent fit`}>{job.fit_score}%</div> : null}</div>
